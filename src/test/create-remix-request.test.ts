...
describe('createRemixRequest', () => {
  it('converts an APIGatewayProxyEventV2 to a Remix Request', () => {
    ...
    const remixRequest = createRemixRequest(event);

    expect(remixRequest.method).toStrictEqual('GET');
    expect(remixRequest.headers.get('host')).toStrictEqual(
      'abcdef1234.execute-api.us-east-1.amazonaws.com',
    );
    expect(remixRequest.url).toStrictEqual(
      'https://abcdef1234.execute-api.us-east-1.amazonaws.com/?a=1&b=2%2C3&c%5B%5D=4&c%5B%5D=5',
    );
  });
});
